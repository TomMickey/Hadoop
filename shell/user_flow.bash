...
v_hour=`date '+%T'` #当前时间  精确到时分秒 hh:mm:ss
v_second=60
v_leave_minute=15
v_leave_second=`expr $v_leave_minute \* $v_second`
v_start_min=`expr $v_second - $v_leave_minute`
#v_last_hour=$v_hour:00:00 #时间段 小
v_acct_hour=`date +%H`
v_account_hour=`date +%H`:00:00 
v_time=`date +%T` #当前时间 00:00:00 

v_data_file=/data2/USERS/hadoop_dfgx_A/zk/20180702-fs_xzzx/data/fs_user_flow.txt #数据存放文件
v_log_file=/data2/USERS/hadoop_dfgx_A/zk/20180702-fs_xzzx/log/fs_user_flow.log #日志记录文件
...
hive -e "
	...
	select g.xzzx_id,g.xzzx_name,'$v_hour' as update_time,nvl(cast(round(f.users/h.xishu,0) as int),0) as users, '$v_date' as day_id from 
				   ...
				   (select bb.xzzx_id,bb.xzzx_name,b.day_id,b.isdn,td(b.time,nvl(b.next_time,b.time)) as stay_time from 
					  (select  a.day_id,a.isdn,a.lac,a.ci,a.time,lead(a.time) over(partition by a.isdn order by a.time) as next_time from
						(select concat(aa.year,lpad(aa.month,2,0),lpad(aa.day,2,0)) day_id,aa.isdn,aa.lac,aa.ci,
							 ...
							 and regexp_replace(aa.time,'-',':')>='07:$v_start_min:00' and regexp_replace(aa.time,'-',':')<'$v_account_hour' 
						) a  
					   ) b 
...
append INTO TABLE FSAC_XZZX_USER_FLOW \n
...
"
v_ctl_file=/data2/USERS/hadoop_dfgx_A/zk/20180702-fs_xzzx/shell/fsac_xzzx_user_flow.ctl
v_ctl_log=/data2/USERS/hadoop_dfgx_A/zk/20180702-fs_xzzx/shell/fsac_xzzx_user_flow.log
